...
if __name__ == '__main__':
    ...
    for i in range(len(start_time_list)):
        ...
        # # 下面是包含路中卡口的4个子路段
        # query_res = plate_match(conn, ['F021', 'HK-89'],[('1','2','3'), ('7','8','9','10','11')], start_time_list[i], end_time_list[i])   # 起：HK-89；终：F021。跑该子段时取消注释
        # query_res = plate_match(conn, ['HK-1931', 'F021'], [('1', '2', '3'), ('1','2','3')], start_time_list[i],
        #                         end_time_list[i])     # 起：F021；终：HK-1931。跑该子段时取消注释

        # query_res = plate_match(conn, ['F022', 'HK-1931'], [('1', '2','3'), ('4', '5', '7', '8', '9')], start_time_list[i],
        #                         end_time_list[i])   # 起：HK-1931；终：F022。跑该子段时取消注释

        # query_res = plate_match(conn, ['HK-89', 'F022'], [('1', '2', '3', '4'), ('1', '2','3')],
        #                         start_time_list[i],
        ...
        print('day: ', i)
        # # 下面是包含路中卡口的4个子路段
        # Insert_db(conn, 'TRAVEL_TIME_HK89TOF021', result)    # 起：HK-89；终：F021。跑该子段时取消注释
        # Insert_db(conn, 'TRAVEL_TIME_F021TOHK1931', result)      # 起：F021；终：HK-1931。跑该子段时取消注释
        # Insert_db(conn, 'TRAVEL_TIME_HK1931TOF022', result)      # 起：HK-1931；终：F022。跑该子段时取消注释
        # Insert_db(conn, 'TRAVEL_TIME_F022TOHK89', result)  # 起：F022；终：HK-89。跑该子段时取消注释
