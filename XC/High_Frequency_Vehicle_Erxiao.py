...
def High_frequency_vehicles(conn, start_time):
    ...
    # # 东向西方向，下面查询取消注释
    # query_sql = ("SELECT HPHM, COUNT(JGSJ) FROM SJCJ_T_CLXX_LS_ERXIAO WHERE SSID='F022' AND CDBH IN ('1','2','3') "
    #              "AND TO_CHAR(JGSJ,'HH24') IN ('15','16','17')"
    ...
    # 西向东方向，下面查询取消注释
    query_sql = ("SELECT HPHM, COUNT(JGSJ) FROM SJCJ_T_CLXX_LS_ERXIAO WHERE SSID='F021' AND CDBH IN ('1','2','3') "
                 "AND TO_CHAR(JGSJ,'HH24') IN ('15','16','17')"
    ...
    holiday_list = Get_Holidays_during_Aweek(
        start_time)  # 根据给定的start_time，生成该周的休息日 eg：['2019-05-011 00:00:00', '2019-05-12 00:00:00']

    holiday_query_list = []  # 将休息日的查询结果转化为dataframe格式，存入该列表
    for i in range(len(holiday_list)):
        # # 东向西方向，下面查询取消注释
        # query_sql_for_holiday = ("SELECT HPHM, COUNT(JGSJ) FROM SJCJ_T_CLXX_LS_ERXIAO WHERE SSID='F022' AND CDBH IN ('1','2','3') "
        #          "AND TO_CHAR(JGSJ,'HH24') IN ('15','16','17')"
        ...
        query_sql_for_holiday = (
                                    "SELECT HPHM, COUNT(JGSJ) FROM SJCJ_T_CLXX_LS_ERXIAO WHERE SSID='F021' AND CDBH IN ('1','2','3') "
                                    "AND TO_CHAR(JGSJ,'HH24') IN ('15','16','17')"
                                    " AND JGSJ BETWEEN to_date('%s','yyyy-mm-dd hh24:mi:ss') AND to_date('%s','yyyy-mm-dd hh24:mi:ss') GROUP BY HPHM ") % (
    ...
    df_holiday_total_tem = pd.concat([dataframe_res_total, df_holiday], ignore_index=True,
    ...
    df_holiday_total['WORK_numbers'] = df_holiday_total['TOTAL_numbers'].sub(
        df_holiday_total['HOLIDAY_numbers'])  # TOTAL_numbers列减去HOLIDAY_numbers列，赋值给新增的WORK_numbers列
    df_holiday_total['START_DATE'] = datetime.datetime.strptime(query_time_interval[0],
                                                                '%Y-%m-%d %H:%M:%S')  # 新增一列用于表征计算起始的日期
    df_holiday_total['END_DATE'] = datetime.datetime.strptime(query_time_interval[1],
                                                              '%Y-%m-%d %H:%M:%S')  # 新增一列用于表征统计截止的日期（不包括当天）
    ...
    df_holiday_total = df_holiday_total[
        ['HPHM', 'TOTAL_numbers', 'WORK_numbers', 'HOLIDAY_numbers', 'START_DATE', 'END_DATE']]  # 提取对应列重构dataframe
