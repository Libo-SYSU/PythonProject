...
def plate_match(conn, SSID, CDBH, start_time, end_time):
    ...
    query_plate_low = (
                          "SELECT HPHM, HPZL, JGSJ FROM SJCJ_T_CLXX_LS_LIUZHONG WHERE SSID='%s' AND CDBH in %s AND JGSJ>=to_date('%s','yyyy-mm-dd hh24:mi:ss') AND JGSJ<=to_date('%s','yyyy-mm-dd hh24:mi:ss')") % (
