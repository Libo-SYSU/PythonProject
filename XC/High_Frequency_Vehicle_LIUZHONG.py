...
def High_frequency_vehicles(conn, start_time):
    ...
    query_sql = ("SELECT HPHM, COUNT(JGSJ) FROM SJCJ_T_CLXX_LS_LIUZHONG WHERE SSID='HK-107' AND CDBH IN ('1','2','3','4') "
    ...
    for i in range(len(holiday_list)):
        query_sql_for_holiday = ("SELECT HPHM, COUNT(JGSJ) FROM SJCJ_T_CLXX_LS_LIUZHONG WHERE SSID='HK-107' AND CDBH IN ('1','2','3','4') "
                 "AND TO_CHAR(JGSJ,'HH24') IN ('16','17')"
...
if __name__ == '__main__':
    ...
    for i in range(len(start_time_list)):
        ...
        result = dataframe_Tolist(df_holiday_total)
        print('week: ', i)
        Insert_db(conn, 'HIGH_FRE_VEHICLES_LIUZHONG', result)
